...
def swod_analysis(request):
    ...
    attendances = UserAttendance.objects.filter(
        user__in=users, date__range=[from_date, to_date]
    )
    # Get all the tasks
    ...
    for key, value in project_avg.items():
        if value > company_total_avg:
            how_much_above = value - company_total_avg
            project_above_avg[key] = round(how_much_above, 2)
    # add how much above average to each user total
    users_bonus = {}
    for user in users:
        if user.current_project:
            users_bonus[user.id] = project_above_avg[user.current_project.id]
            user_totals[user.id] += project_above_avg[user.current_project.id]
    ...
    j_max = max(j_user_totals.values())
    m_max = max(m_user_totals.values())
    s_max = max(s_user_totals.values())
    ...
    context = {
        ...
        "company_total_avg": company_total_avg,
